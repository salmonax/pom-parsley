...
/**
 * Takes a string and build up a data object in PomParsley format
 * @param (string) pomsheet
 */

module.exports = function buildParsleyData(pomsheet) {
  var lines = pomsheet.split(/\n/);
  var currentDate;
  var parsley = { 
    lines: lines, 
    tasks: [], 
  ...
  function parseTask(line,index) {
    ...
    var categories, category, subcategory, description;
    ...
    function checkTag(maybeTag) {
      var definedTags = Object.keys(parsley.tags).map((tag) => tag.match(/\w/) ? tag : '\\' + tag).join('');
